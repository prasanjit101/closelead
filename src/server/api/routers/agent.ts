...
export const agentRouter = createTRPCRouter({
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.webhookIds.length > 0) {
        await ctx.db.insert(agentWebhooks).values(
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (webhookIds !== undefined) {
        ...
        await ctx.db.delete(agentWebhooks).where(eq(agentWebhooks.agentId, id));
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await ctx.db.delete(agent).where(eq(agent.id, input.id));
